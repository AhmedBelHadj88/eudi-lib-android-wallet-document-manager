...
class DocumentManagerImpl(
    ...
    override fun storeIssuedSDJWTDocument(
        unsignedDocument: UnsignedDocument,
        sdJwt: SdJwtVerifiableCredential,
    ): StoreDocumentResult {
        try {
/*
            val timeSigned = Clock.System.now()
            val timeValidityBegin = timeSigned.plus(1.hours)
            val timeValidityEnd = timeSigned.plus(10.days)
            ...
            val document = documentStore.createDocument(unsignedDocument.name)
            val credential = SdJwtVcCredential(
                document,
                null,
                "domain",
                secureArea,
            ...
            credential.certify(sdJwt.toString().toByteArray(), timeValidityBegin, timeValidityEnd)
            return StoreDocumentResult.Success(credential.documentName, null)*/
            return  StoreDocumentResult.Success("credential.documentName", null)
        } catch (e: Exception) {
            return StoreDocumentResult.Failure(e)
        }
